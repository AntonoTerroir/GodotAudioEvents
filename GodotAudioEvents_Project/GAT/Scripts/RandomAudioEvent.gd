extends AudioEvent
class_name RandomAudioEvent, "res://GAT/Icons/RandomAudioEvent.png"
...
func stop(id = -1):
	...
	if(id < 0 || id > (_audio_children.size() - 1)):
		...
		if(fade_out_active):
			...
			_audio_children[id].stop()
			_audio_children.remove(id)
...
func _get_random_child():
	randomize()
	var _soundID = randi()%_audio_children.size()
	while(_soundID == _lastRandomSound):
		_soundID = randi()%_audio_children.size()
	_lastRandomSound = _soundID
	var _randomSound = _audio_children[_soundID]
	return _randomSound
