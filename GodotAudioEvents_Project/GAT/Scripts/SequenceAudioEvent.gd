extends AudioEvent
class_name SequenceAudioEvent, "res://GAT/Icons/SequenceAudioEvent.png"
...
func _on_ready():
	_event = _audio_children[_cur_index]
...
func play(id = -1):
	...
	if(id > -1 && id < (_audio_children.size() - 1)):
		_event = _audio_children[id]
	else: 
		_event = _audio_children[_cur_index]
	...
	if _reverse:
		if _cur_index == -1:
			_reverse = false
			if(_audio_children.size() > 1):
				_cur_index = 1
			else:
				_cur_index = 0
			return 
	else:
		if _cur_index < _audio_children.size(): return
...
func set_sequence_end_mode(mode: int):
	on_sequence_end = mode
	
func get_sequence_end_mode():
	return on_sequence_end
